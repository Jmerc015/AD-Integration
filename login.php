...
if(isset($_POST['thecount']) && isset($_POST['username']) && isset($_POST['password']))
{
	...
	$postvar_username = $_POST["username"]; //remove case sensitivity on the username ? (//$username = strtoupper($_POST["username"]);)
...
if (($postvar_username != "") AND (isset($postvar_password))) { //prevent null bind

    if ($postvar_username != NULL && $postvar_password != NULL){
        ...
        if ($adldap->authenticate($postvar_username, $postvar_password)){   

          $emailAdd = $postvar_username . $email_key;
          $checker = isCool($emailAdd);
